...
param(
    [Parameter(HelpMessage = "Perform OMI version upgrade.  If this is not set, the tool will display Linux VMs and show vulnerability status.")]
    [switch]$upgradeOMI = $false
)

Write-Host "UpgradeOMI was set to "$upgradeOMI"."

if ($upgradeOMI -eq $false) {
    Write-Host "Printing vulnerability status for all VMs that the user has access to."
    Write-Warning "Run this script with the -upgradeOMI flag in order to actually patch vulnerable machines."
}

if ($upgradeOMI -eq $true) {
    Write-Host "The OMI agent will be updated on all vulnerable Azure Linux VMs that the user running the script has access to."
}
...
if ($upgradeOMI -eq $false) {
    Write-Warning "Reminder: Run this script with the -upgradeOMI flag in order to actually patch vulnerable machines!"
}